...
`tools/list` at `https://gateway.pipeworx.io/studio-ghibli/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/films \
  -H 'Content-Type: application/json' \
  -d '{"limit":10}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/films`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "studio-ghibli": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-studio-ghibli"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-studio-ghibli
```
